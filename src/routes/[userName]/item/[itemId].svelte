...
{:else}
	...
	<div class="flex flex-row gap-14 justify-center pb-8">
		<div class="card w-2/3 bg-gradient-to-b from-base-100 to-base-300 shadow-xl">
			<div class="card-body">
				<div class="divider">
					...
					{#each priceData.buyFor as buyOffer}
						{#if buyOffer.vendor.name !== 'Flea Market'}
							<div class="flex flex-row gap-1 justify-center">
								...
								<span class="text-accent font-bold w-1/2"
									>{getTrader(buyOffer.vendor.name).name}</span
								>
		...
		<div class="flex w-1/3">
			<div class="card bg-gradient-to-b from-base-100 to-base-300 shadow-xl">
				...
				<div class="card-body flex pt-4">
					...
					<span class="flex justify-center text-accent text-sm"
						>{item.width + 'x' + item.height}</span
					>
					<!-- All tasks that rewards the item -->
					<ul class="list-disc list-inside py-2">
						{#each item.TaskRewardsItem as taskRewards}
							<li class="text-accent text-xs">
								...
								{#if taskRewards.task.PlayerHasTasks[0].completed}
									<span class="text-success font-extrabold cursor-help" title="Completed"
										>&check;</span
									>
								{/if}
							</li>
	...
	{#if item.TaskReqItem.length || item.HideoutReqItem.length}
		<div class="pb-8">
			<div
				class="card w-full flex-wrap justify-center bg-base-100 shadow-xl lg:card-side bg-gradient-to-b even:from-base-100 odd:from-base-200 to-base-300"
			>
						...
						<table class="table table-auto table-zebra w-full">
							...
							<thead>
								<tr class="border border-b-[1px] border-primary border-x-0 border-y-0">
									...
									<th class="text-primary w-1/12  ">
										<div class="flex justify-center text-primary">Completed</div>
									</th>
								</tr>
							</thead>
							<tbody>
								...
								{#each item.TaskReqItem as reqItem}
									<tr>
										...
										<td>
											{#if reqItem.task.PlayerHasTasks[0].completed}
												<div class="text-success flex justify-center font-extrabold">&check;</div>
											{/if}
										</td>
									</tr>
								{/each}
								...
								{#each item.HideoutReqItem as reqItem}
									<tr>
										<td
											class="text-success font-bold flex justify-center"
											title="Item can be bought">{reqItem.count}</td
										>
										<td>
											<div class="text-accent flex justify-center">
										...
										<td>
											{#if reqItem.hideoutStation.PlayerHasHideout[0].completed}
												<div class="text-success flex justify-center font-extrabold">&check;</div>
											{/if}
										</td>
									</tr>
	...
	{#if item.CraftReqItem.length || item.CraftRewItem.length}
		<div class="pb-8">
			<div
				class="card w-full flex-wrap justify-center bg-gradient-to-b from-base-100 to-base-300 shadow-xl lg:card-side"
			>
				<div class="flex w-full bg-primary justify-center">
